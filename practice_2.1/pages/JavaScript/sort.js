const createSortArr = (data) => {
    ...
    const fields = [
        { selectId: 'fieldsFirst',  descId: 'fieldsFirstDesc'  },
        { selectId: 'fieldsSecond', descId: 'fieldsSecondDesc' },
        { selectId: 'fieldsThird',  descId: 'fieldsThirdDesc'  }
    ];
    for (const field of fields) {
        ...
        const checkbox = document.getElementById(field.descId);
        const desc = checkbox ? checkbox.checked : false;
...
const sortTable = (idTable, formData) => {
    ...
    const hasSorting = sortArr.length > 0;
    if (!hasSorting) {
